"""
File: dbt_gold.py
Description: DAGs orchestration for dbt Gold (Marts) layer using Astronomer Cosmos.
"""
...
# Khai báo cấu hình luồng chạy chuyên biệt cho tầng Gold.
# =======================================================================
dag_configs = [
    {
        "dag_id": "dbt_gold_incremental",
        "is_full_refresh": False,
        "description": "Incremental pipeline cho tầng Gold (Chạy hàng ngày)"
    },
    {
        "dag_id": "dbt_gold_full_load",
        "is_full_refresh": True,
        "description": "Full Refresh pipeline cho tầng Gold (Drop và Rebuild toàn bộ Marts)"
    }
]
...
for config in dag_configs:
    with DAG(
        ...
        schedule_interval=None,  # Để None để Sensor hoặc TriggerDagRunOperator gọi tới
        start_date=datetime(2023, 1, 1),
        catchup=False,
        is_paused_upon_creation=False,
        tags=["lakehouse", "dbt", "trino", "gold", "marts"], # Đã update tags chỉ dành cho Gold
        description=config["description"]
    ) as dag:

        # =======================================================================
        # TASK GROUP: TẦNG GOLD (Marts)
        # =======================================================================
        transform_gold = DbtTaskGroup(
            ...
            # RenderConfig trỏ thẳng vào thư mục marts
        ...
        # Do chỉ còn 1 layer nên không cần thiết lập dependency (như silver >> gold nữa)
        # DAG chỉ chạy duy nhất transform_gold
